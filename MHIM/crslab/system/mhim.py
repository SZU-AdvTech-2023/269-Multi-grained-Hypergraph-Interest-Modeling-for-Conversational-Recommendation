...
class MHIMSystem(BaseSystem):
    ...
    def __init__(self, opt, train_dataloader, valid_dataloader, test_dataloader, vocab, side_data, restore_system=False,
        ...
        print("recommend_epoch:", self.rec_epoch)
        print("conversation_epoch:", self.conv_epoch)
        print("self.rec_batch_size: ", self.rec_batch_size)
        print("self.conv_batch_size: ", self.conv_batch_size)
    ...
    # def txt2ind(user_input):
    #     # 分词
    #     tokens = user_input.split()
    #     # 将分词转换为索引
    #     indices = [ind2txt[token] for token in tokens if token in ind2txt]
    #     return indices


    def interact(self):
        print("交互模式 - 输入 'exit' 以结束。")
        while True:
            user_input = input("你: ")
            ...
            # 将用户输入转换为模型输入格式（分词，转换为张量等）
            # 分词
            tokens = user_input.split()
            # 将分词转换为索引
            user_input_indices = [self.tok2ind[token] for token in tokens if token in self.tok2ind]
            user_input_tensor = torch.tensor(user_input_indices, device=self.device).unsqueeze(0)
            ...
            with torch.no_grad():
                ...
                rec_scores = self.model.rec_model(user_input_tensor, mode='test')
                conv_preds = self.model.conv_model(user_input_tensor, mode='test')

            # 将模型输出转换为人类可读的格式
            recommended_item = self.item_ids[torch.argmax(rec_scores).item()]
            response_str = ind2txt(conv_preds.squeeze().tolist(), self.ind2tok, self.end_token_idx)

            print(f"模型: 推荐项目 {recommended_item}")
            print(f"模型: 生成的回应: {response_str}")
